...
//DECLARE_MUTEX(lock1); /*Compile time initialization of semaphore, mutex form. initially unloked */
//DECLARE_MUTEX has been renamed to DEFINE_SEMAPHORE
DEFINE_SEMAPHORE(lock1); /*Compile time initialization of semaphore, mutex form. initially unloked */
//DECLARE_MUTEX_LOCKED(lock3); /*Compile time initialization of semaphore, mutex form. initially loked */
...
static int __init registration(void) {
    int ret = 0;
    sema_init(&lock2, 1); /*Runtime initialization of semaphore, mutex form.*/
...
static int scull_open(struct inode *inod, struct file *fil) {
    ...
    if(down_interruptible(&lock1)) {
        printk(KERN_ERR "Failed to lock while opening scull.\n"); /*Does not take lock when this occurs*/
        return -ERESTARTSYS;
    }
...
static ssize_t scull_read(struct file *filp, char *buff, size_t len, loff_t *off) {
    ...
    up(&lock1); //lock is being released in scull_read for testing purpose
...
static ssize_t scull_write(struct file *filp, const char *buff, size_t len, loff_t *off) {
    ...
    if(down_interruptible(&lock1)) {
        printk(KERN_ERR "Failed to lock2 while opening scull.\n"); /*Does not take lock when this occurs*/
        return -ERESTARTSYS;
    }
